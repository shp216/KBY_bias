        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.evaluation_step==0:      
                    eval_gender_images(args, accelerator, tokenizer, text_encoder, unet, vae, eval_noise_scheduler)
                    accelerator.wait_for_everyone()
                    
                    if accelerator.is_main_process:
                        # eval-generated-images.py 의 parse_args 함수 호출
                        evaluate_biased_score(args, accelerator)
                        time.sleep(5)
                        # 평가 완료 후 디렉토리 삭제
                ...
                if global_step % args.valid_steps==0:      
                    ...
                    if accelerator.is_main_process:
                        # eval-generated-images.py 의 parse_args 함수 호출
                        evaluate_biased_score(args, accelerator, mode="eval_images")
                        time.sleep(5)
                        #평가 완료 후 디렉토리 삭제
                        if os.path.exists(args.eval_save_dir):
                            shutil.rmtree(args.eval_save_dir)
