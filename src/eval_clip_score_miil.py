...
def evaluate_clip_score(args, accelerator, i=None):
    ...
    num_files = len(file_list)
    print(num_files)
    indices = torch.tensor_split(torch.arange(num_files), world_size)
    local_file_list = [file_list[i.item()] for i in indices[rank]]
    score_arr = []
    ...
    return final_score, float(variance_score)
