...
def parse_args():
    ...
    parser.add_argument("--unet_config_name", type=str, default="teacher", choices=["teacher", "bk_base", "bk_small", "bk_tiny","original"])   
    ...
    parser.add_argument("--data_list", type=str, default="./data/test_occupation_50.csv")
    ...
    parser.add_argument(
        "--eval_save_dir_256",
        type=str,
        default="./outputs/test_images_256",
    )
    parser.add_argument(
        "--test_results_dir",
        type=str,
    ...
    args.eval_score_logdir = os.path.join(args.output_dir, "eval_scores")
    os.makedirs(args.eval_score_logdir, exist_ok=True)

    args.eval_image_logdir = os.path.join(args.output_dir, "eval_images")
    os.makedirs(args.eval_image_logdir, exist_ok=True)

    args.eval_save_dir = os.path.join(args.output_dir, "outputs/test_images")
    args.eval_save_dir_256 = os.path.join(args.output_dir, "outputs/test_images_256")
    args.test_results_dir = os.path.join(args.output_dir, "outputs/test_results")
...
def main():
    ...
    eval_noise_scheduler = DPMSolverMultistepScheduler.from_config(
    ...
    if args.unet_config_name in ['original']:
        ...
        mapping_layers = ['up_blocks.0.upsamplers.0.conv', 'up_blocks.1.upsamplers.0.conv', 'up_blocks.2.upsamplers.0.conv', 'up_blocks.3.attentions.2.proj_out',
                          'mid_block.attentions.0.proj_out', 'down_blocks.0.downsamplers.0.conv', 'down_blocks.1.downsamplers.0.conv', 'down_blocks.2.downsamplers.0.conv', 'down_blocks.3.resnets.1.conv2']    
...
#########################################################################################################################################################################################
    ...
    logger.info(f"  Total gpu numbers = {world_size}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                loss_reg = torch.tensor(0.0, device=accelerator.device)
                if args.use_reg_loss:
                    with torch.no_grad():
                        teacher_params = dict(unet_teacher.named_parameters())

                    for name, param in unet.named_parameters():
                        if name in teacher_params and param.shape == teacher_params[name].shape:
                            loss_reg += F.mse_loss(param.float(), teacher_params[name].float(), reduction='sum')
                ...
                #     for name, param in unet.named_parameters():
                #         if name in teacher_params and param.shape == teacher_params[name].shape:
                #             diff = param - teacher_params[name]
                #             loss_reg += torch.sum((diff.float()) ** 2)  # L2^2
                #             total_elements += diff.numel()

                #     if total_elements > 0:
                #         loss_reg = loss_reg / total_elements  # 평균 L2 제곱 거리


                # Compute the final loss
                loss = args.lambda_sd * loss_sd + args.lambda_kd_output * loss_kd_output + args.lambda_kd_feat * loss_kd_feat + args.lambda_reg * loss_reg
                ...
                avg_loss_reg = accelerator.gather(loss_reg.repeat(args.train_batch_size)).mean()
                train_loss_reg += avg_loss_reg.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.valid_steps==0:      
                    ...
                    if accelerator.is_main_process:
                        # eval-generated-images.py 의 parse_args 함수 호출
                        step_image_dir = os.path.join(args.eval_image_logdir, str(global_step))
                        os.makedirs(step_image_dir, exist_ok=True)
                        evaluate_biased_score(args, accelerator, mode="eval_images", global_step=global_step)
                        time.sleep(5)
                        #평가 완료 후 디렉토리 삭제
                        if os.path.exists(args.eval_save_dir):
                            shutil.rmtree(args.eval_save_dir)
