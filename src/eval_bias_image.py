...
def eval_gender_images(args, accelerator, tokenizer, text_encoder, unet, vae, noise_scheduler, mode="eval_score"):
    ...
    for i, prompt_i in tqdm(enumerate(local_prompts), total=len(local_prompts), desc='Generating Images', leave=True):
        ...
        save_dir_prompt_i = os.path.join(args.eval_save_dir, f"prompt_{i}")
    ...
    change_img_count = change_img_size_bias(args.eval_save_dir, args.eval_save_dir_256, args.img_resz, accelerator)
