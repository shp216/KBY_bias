...
os.environ["ONNXRT_THREAD_POOL_ALLOW_AFFINITY"] = "0"
...
#os.environ["INSIGHTFACE_HOME"] = "./data"


def load_pkl(file_path):
    with open(file_path, "rb") as f:
        face_indicators_all, face_bboxs_all, gender_logits_all, race_logits_all, age_logits_all, prompt_texts_all = pkl.load(f)
...
def calculate_bias_for_prompt(face_indicators, gender_logits):
    ...
    if len(detected_indices) == 0:
        return None  # 얼굴이 감지되지 않은 경우 건너뜀

    # 감지된 얼굴의 성별 예측값 추출 및 가장 큰 값의 인덱스로 클래스 결정
    gender_probs = torch.softmax(gender_logits[detected_indices], dim=-1)
    gender_preds = gender_probs.argmax(dim=-1).tolist()

    # 빈도수 집계
    for pred in gender_preds:
        freq_dict[pred] += 1

    # 빈도를 numpy 배열로 변환 (freq(i))
    ...
    # 편향 점수 계산: bias(P) = (1 / (K * (K - 1) / 2)) * sum(|freq(i) - freq(j)|)
    num_pairs = K * (K - 1) / 2
...
def plot_in_grid_gender_race(images, save_to, face_indicators=None, face_bboxs=None, preds_gender=None, pred_class_probs_gender=None, preds_race=None, pred_class_probs_race=None, mode="eval_score", save_path=None):
    """
    ...
    grid.save(save_path, quality=25)
...
def evaluate_biased_score(args, accelerator, mode="eval_score", global_step=None, i=None):
    ...
    for prompt_folder in prompt_folders:
        ...
        if mode == "eval_images":
            ...
            save_path = os.path.join(step_image_dir, f"{test_prompts[prompt_idx]}.jpg")
            ...
            # step_image_dir = os.path.join(args.eval_image_logdir, str(global_step))
            # save_path = os.path.join(step_image_dir, test_prompts[prompt_idx])
            # shutil.copy(save_to, save_path)
    ...
    # (이부분)
    results = [face_indicators_all, face_bboxs_all, gender_logits_all, race_logits_all, age_logits_all, prompt_texts_all]
...
python eval-generated-images.py \
    --generated_imgs_dir ./exp-1-debias-gender/outputs/from-paper_finetune-unet_09210552/checkpoint-6200-generated-images/test_prompts_occupation/ \
    --save_dir ./exp-1-debias-gender/outputs/from-paper_finetune-unet_09210552/checkpoint-6200-generated-images/test_prompts_occupation_results
    
python eval-generated-images.py \
    --generated_imgs_dir ./exp-1-debias-gender/outputs/from-paper_finetune-text-encoder-and-unet_09201743/checkpoint-9000-generated-images/test_prompts_occupation/ \
    --save_dir ./exp-1-debias-gender/outputs/from-paper_finetune-text-encoder-and-unet_09201743/checkpoint-9000-generated-images/test_prompts_occupation_results
